import torch  # 导入 PyTorch，用来定义 LSTM 模型和张量；这里没有张量 shape。


class LSTMRegressor(torch.nn.Module):  # 定义一个最小 LSTM 回归模型；输入是序列，输出是一个回归值。
    def __init__(self, input_size: int = 1, hidden_size: int = 8) -> None:  # 初始化模型参数；input_size 和 hidden_size 都是整数。
        ...
        self.lstm = torch.nn.LSTM(input_size=input_size, hidden_size=hidden_size, batch_first=True)  # 定义单层 LSTM，输入 shape = (B, T, 1)，输出 shape = (B, T, hidden_size)。
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # 定义前向传播；输入序列 x 的 shape = (B, T, 1)。
        out, (hidden, cell) = self.lstm(x)  # 输入 LSTM，out 的 shape = (B, T, hidden_size)，hidden 和 cell 的 shape = (1, B, hidden_size)。
...
model = LSTMRegressor(input_size=1, hidden_size=8)  # 创建 LSTM 回归模型；模型本身没有 shape。
