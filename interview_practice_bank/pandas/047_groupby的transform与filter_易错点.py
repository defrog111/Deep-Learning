"""
题目 047：GroupBy的transform与filter_易错点

要求：完成“GroupBy的transform与filter”的易错点题，并解释输出的 shape、索引和数据类型。
...
large_groups = frame.groupby('team').filter(lambda group: group['score'].mean() >= 55)  # filter按组保留原始行。
