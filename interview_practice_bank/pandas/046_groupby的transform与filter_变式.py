"""
题目 046：GroupBy的transform与filter_变式

要求：完成“GroupBy的transform与filter”的变式题，并解释输出的 shape、索引和数据类型。
...
large_groups = frame.groupby('team').filter(lambda group: group['score'].mean() >= 54)  # filter按组保留原始行。
