...
要求：完成“PCA与解释方差”的基础题，说明fit、transform和predict各自只能使用哪些数据。
...
pipeline = make_pipeline(StandardScaler(), PCA(n_components=3, svd_solver='full'))  # 标准化后使用完整SVD降维。
projected = pipeline.fit_transform(features)  # 拟合主成分并投影。
pca = pipeline.named_steps['pca']  # 取得已拟合PCA步骤。
assert projected.shape[1] == 3 and pca.explained_variance_ratio_.sum() <= 1  # 验证降维shape和解释方差。
