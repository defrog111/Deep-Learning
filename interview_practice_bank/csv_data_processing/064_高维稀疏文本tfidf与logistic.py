...
from sklearn.metrics import accuracy_score, f1_score, log_loss, precision_score, recall_score  # 导入多分类和概率质量指标。
...
train_frame = frame.query("split == 'train'").copy()  # 从清洗后的数据取出训练文本和标签。
val_frame = frame.query("split == 'val'").copy()  # 从清洗后的数据取出验证文本和标签。
...
val_precision = precision_score(val_frame['category'], val_predictions, average='macro', zero_division=0)  # 计算各类别precision的宏平均。
val_recall = recall_score(val_frame['category'], val_predictions, average='macro', zero_division=0)  # 计算各类别recall的宏平均。
val_f1 = f1_score(val_frame['category'], val_predictions, average='macro', zero_division=0)  # 计算各类别F1的宏平均。
...
print('sparse_shape:', train_matrix.shape, 'stored_nonzero_values:', train_matrix.nnz, 'validation_accuracy:', val_accuracy, 'validation_precision_macro:', val_precision, 'validation_recall_macro:', val_recall, 'validation_f1_macro:', val_f1, 'validation_log_loss:', val_log_loss, 'top_terms:', top_terms, 'inference:', inference_result, sep='\n')  # 输出稀疏结构、分类指标、关键词和推理结果。
