...
def make_supervised_data(partition):  # 定义从连续值构造下一方向分类样本的函数。
    ...
    targets = (next_values > values[:, -1]).long()  # 创建真实下一方向类别。
    return inputs, targets  # 返回类别窗口和下一类别标签。
...
class GRUNextClass(nn.Module):  # 定义两层GRU下一类别模型。
    def __init__(self):  # 初始化模型组件。
        super().__init__()  # 初始化nn.Module父类。
        self.embedding = nn.Embedding(len(class_names), 8)  # 把涨跌类别编号转换为向量。
        self.gru = nn.GRU(input_size=8, hidden_size=16, num_layers=2, batch_first=True)  # 使用两层GRU编码五类别窗口。
        self.head = nn.Linear(16, len(class_names))  # 输出下一方向的两个logit。
    def forward(self, class_window):  # 定义单步下一类别预测。
        _, hidden = self.gru(self.embedding(class_window))  # 获取最后一层GRU最终隐藏状态。
        return self.head(hidden[-1])  # 根据完整历史类别窗口预测下一类别。
    def generate(self, seed_window, horizon):  # 定义可配置步数的递归类别生成。
        ...
        for _ in range(horizon):  # 按forecast_horizon逐步生成未来类别。
            ...
            generated.append(next_class)  # 保存本轮预测类别。
...
torch.manual_seed(42)  # 固定初始化以保证结果可复现。
model = GRUNextClass()  # 实例化两层GRU下一类别模型。
...
with torch.inference_mode():  # 关闭梯度执行验证与递归推理。
    val_predictions = model(val_x).argmax(dim=1)  # 预测验证集下一方向类别。
    val_accuracy = (val_predictions == val_y).float().mean()  # 计算一步验证准确率。
    inference_indices = model.generate(inference_x, forecast_horizon)  # 递归生成指定数量的未来类别。
...
# 递归预测的第二步开始会使用模型自己的前一步类别，因此未来步数越多误差传播越明显。
