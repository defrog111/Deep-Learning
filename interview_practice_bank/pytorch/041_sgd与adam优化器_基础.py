"""
题目 041：SGD与Adam优化器_基础

要求：完成“SGD与Adam优化器”的基础题，说明训练态、梯度和张量shape。
...
optimizers = [torch.optim.SGD(model_sgd.parameters(), lr=0.03), torch.optim.Adam(model_adam.parameters(), lr=0.01)]  # 配置两种优化器。
