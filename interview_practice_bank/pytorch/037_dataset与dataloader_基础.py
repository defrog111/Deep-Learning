"""
题目 037：Dataset与DataLoader_基础

要求：完成“Dataset与DataLoader”的基础题，说明训练态、梯度和张量shape。
...
loader = DataLoader(dataset, batch_size=2, shuffle=True, generator=torch.Generator().manual_seed(0))  # 创建可复现的随机batch。
batch_features, batch_labels = next(iter(loader))  # 取得第一个batch。
assert len(dataset) == 10 and len(batch_features) <= 2  # 验证样本数和batch大小。
